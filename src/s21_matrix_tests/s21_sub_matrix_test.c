...
START_TEST(s21_sub_matrix_test_6) {
  ...
  ck_assert_double_eq(C.matrix[0][1], 0.0);
  ck_assert_double_eq(C.matrix[1][0], 0.0);
  ck_assert_double_eq(C.matrix[1][1], 0.0);
  s21_remove_matrix(&A);
  s21_remove_matrix(&B);
  s21_remove_matrix(&C);
}
...
Suite *s21_sub_matrix_suite(void) {
  TCase *tc_case = tcase_create("sub_matrix");
  Suite *s = suite_create("s21_sub_matrix_test");

  tcase_add_test(tc_case, s21_sub_matrix_test_1);
  tcase_add_test(tc_case, s21_sub_matrix_test_2);
  tcase_add_test(tc_case, s21_sub_matrix_test_3);
  tcase_add_test(tc_case, s21_sub_matrix_test_4);
  tcase_add_test(tc_case, s21_sub_matrix_test_5);
  tcase_add_test(tc_case, s21_sub_matrix_test_6);
